...
const myExtension = __importStar(require("../extension")); // Assuming extension.ts is in src/
const openai_1 = __importDefault(require("openai")); // Import OpenAI to stub its methods
// Helper to create a mock Uri
const fileUri = (path) => vscode.Uri.file(path);
describe('Folder Scanning Functionality (`secure-coding-assistant.scanFolder`)', () => {
    let sandbox;
    let mockContext;
    let readDirectoryStub;
    let statStub; // For checking file types if readDirectory doesn't provide them
    ...
    let getSecretStub;
    let workspaceFoldersStub; // Stub for workspace.workspaceFolders
    let openaiCreateStub; // Stub for OpenAI chat completions
    ...
    beforeEach(() => {
        ...
        openaiCreateStub = sandbox.stub(openai_1.default.Chat.Completions.prototype, 'create');
        ...
        getConfigurationStub.withArgs('secureCodingAssistant.openai').returns({
            get: sandbox.stub().callsFake((key, defaultValue) => {
                ...
                if (key === 'userPrompt')
                    return "Test user prompt: {languageId} {codeSnippet}";
                return defaultValue;
            }),
            has: sinon.stub().returns(true),
            inspect: sinon.stub(),
        ...
        openTextDocumentStub = sandbox.stub(vscode.workspace, 'openTextDocument').callsFake((uri) => {
            ...
            if (path.endsWith('.ts') || path.endsWith('.js') || path.endsWith('.py')) {
                return Promise.resolve({
                    ...
                    lineAt: sinon.stub().callsFake((lineOrPosition) => {
    ...
    describe('scanFolder command: No folderUri provided', () => {
        it('should use the first workspace folder if no folder URI is provided and a workspace is open', async () => {
            ...
            assert.ok(showInformationMessageStub.neverCalledWith("No folder is currently open. Please open a folder to scan."), '"No folder open" message should not be shown');
        ...
        it('should show informational message if no folder URI is provided and no workspace is open', async () => {
            ...
            await vscode.commands.executeCommand('secure-coding-assistant.scanFolder', undefined);
            assert.ok(showInformationMessageStub.calledWith("No folder is currently open. Please open a folder to scan."), '"No folder open" message was not shown');
        ...
        it('should show informational message if no folder URI is provided and workspaceFolders is undefined', async () => {
            ...
            await vscode.commands.executeCommand('secure-coding-assistant.scanFolder', undefined);
            assert.ok(showInformationMessageStub.calledWith("No folder is currently open. Please open a folder to scan."), '"No folder open" message was not shown for undefined workspaceFolders');
    ...
    it('should use custom LLM if configured and API key is present', async () => {
        ...
        const logFound = appendLineSpy.getCalls().some((call) => call.args[0].includes('Scanning file "file_custom.ts" using MyTestLLM'));
    ...
    it('should show error if Custom LLM is selected but none are configured', async () => {
        ...
        assert.ok(showInformationMessageStub.calledWith(sinon.match(/0 file\(s\) scanned successfully, 1 file\(s\) failed/)), "Completion message should indicate failure due to no custom LLM config.");
        const errorLogFound = appendLineSpy.getCalls().some((call) => call.args[0].includes('Error during file scan'));
    ...
    it('should show error if Custom LLM is selected, configured, but API key is missing', async () => {
        ...
        await vscode.commands.executeCommand('secure-coding-assistant.scanFolder', folderUri);
        assert.ok(showInformationMessageStub.calledWith(sinon.match(/0 file\(s\) scanned successfully, 1 file\(s\) failed/)), "Completion message should indicate failure due to missing custom LLM API key.");
        const errorLogFound = appendLineSpy.getCalls().some((call) => call.args[0].includes('API Key for custom LLM "MyTestLLMWithNoKey" not found'));
    ...
    describe('Output Formatting and Field Defaults', () => {
        it('should correctly log fileName and llmProvider for vulnerabilities from OpenAI', async () => {
            ...
            const fileLogFound = appendLineSpy.getCalls().some((call) => call.args[0].includes('Scanning file'));
            assert.ok(fileLogFound, `File and line number log "File: ${testFileName}:10" not found.`);
            // Check for correct logging of llmProvider
            const providerLogFound = appendLineSpy.getCalls().some((call) => call.args[0].includes('using OpenAI'));
